...
@section('content')
    <div class="d-flex justify-content-between shadow-lg p-3 mb-5 bg-body-tertiary rounded">
        <div class="d-flex">
            <p class="rounded bg-danger m-0 d-flex align-self-center p-2 text-white">{{$monsterAttributes->total()}}</p>
            <h1 class="m-0">| Monster attributes</h1>
    ...
    <table class="table table-striped shadow-lg p-3 mb-5 bg-body-tertiary rounded">
        ...
        @foreach($monsterAttributes as $monsterAttribute)
            <tr>
                <td>{{$monsterAttribute->id}}</td>
                <td>{{$monsterAttribute->name}}</td>
                <td>{{$monsterAttribute->created_at ? $monsterAttribute->created_at->diffForHumans() : ''}}</td>
                <td>{{$monsterAttribute->updated_at ? $monsterAttribute->updated_at->diffForHumans() : ''}}</td>
                <td>{{$monsterAttribute->deleted_at ? $monsterAttribute->deleted_at->diffForHumans() : ''}}</td>
                <td>
                    <div class="nav-item dropdown no-arrow">
                        <a class="nav-link dropdown-toggle" href="#" id="userDropdown{{ $monsterAttribute->id }}"
                           role="button"
                        ...
                        <div class="dropdown-menu  shadow "
                             aria-labelledby="userDropdown{{ $monsterAttribute->id }}">
                            @if($monsterAttribute->deleted_at == null)
                                <a class="dropdown-item"
                                   href="{{route('monsterattribute.edit', $monsterAttribute->slug)}}">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                            ...
                            @endif
                            @if($monsterAttribute->deleted_at != null)
                                <form action="{{ route('admin.monsterattributerestore', $monsterAttribute->id) }}"
                                      method="POST">
                                    @csrf
                                    @method('POST')
                            ...
                            @else
                                <form action="{{ route('monsterattribute.destroy', $monsterAttribute->id) }}"
                                      method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="dropdown-item">
    ...
    {{ $monsterAttributes->appends(['search' => Request::get('search'), 'fields' => Request::get('fields')])->links() }}
