    ...
    <table class="table table-striped shadow-lg p-3 mb-5 bg-body-tertiary rounded">
        ...
        @foreach($monsterTypes as $monsterType)
            <tr>
                ...
                <td>{{$monsterType->created_at ? $monsterType->created_at->diffForHumans() : ''}}</td>
                <td>{{$monsterType->updated_at ? $monsterType->updated_at->diffForHumans() : ''}}</td>
                <td>{{$monsterType->deleted_at ? $monsterType->deleted_at->diffForHumans() : ''}}</td>
                <td>
                    <div class="nav-item dropdown no-arrow">
                        <a class="nav-link dropdown-toggle" href="#" id="userDropdown{{ $monsterType->id }}"
                           role="button"
                        ...
                        <div class="dropdown-menu  shadow "
                             aria-labelledby="userDropdown{{ $monsterType->id }}">
                            @if($monsterType->deleted_at == null)
                                <a class="dropdown-item" href="{{route('monstertype.edit', $monsterType->slug)}}">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                            ...
                            @endif
                            @if($monsterType->deleted_at != null)
                                <form action="{{ route('admin.monstertyperestore', $monsterType->id) }}" method="POST">
                                    @csrf
                                    @method('POST')
                            ...
                            @else
                                <form action="{{ route('monstertype.destroy', $monsterType->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="dropdown-item">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
