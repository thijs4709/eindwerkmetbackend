...
@section('content')
    ...
    <table class="table table-striped shadow-lg p-3 mb-5 bg-body-tertiary rounded">
        ...
        @foreach($spellTypes as $spellType)
            <tr>
                <td>{{$spellType->id}}</td>
                <td>{{$spellType->name}}</td>
                <td>{{$spellType->created_at ? $spellType->created_at->diffForHumans() : ''}}</td>
                <td>{{$spellType->updated_at ? $spellType->updated_at->diffForHumans() : ''}}</td>
                <td>{{$spellType->deleted_at ? $spellType->deleted_at->diffForHumans() : ''}}</td>
                <td>
                    <div class="nav-item dropdown no-arrow">
                        <a class="nav-link dropdown-toggle" href="#" id="userDropdown{{ $spellType->id }}"
                           role="button"
                        ...
                        <div class="dropdown-menu  shadow "
                             aria-labelledby="userDropdown{{ $spellType->id }}">
                            @if($spellType->deleted_at == null)
                                <a class="dropdown-item"
                                   href="{{route('spelltype.edit', $spellType->slug)}}">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                            ...
                            @endif
                            @if($spellType->deleted_at != null)
                                <form action="{{ route('admin.spelltyperestore', $spellType->id) }}"
                                      method="POST">
                                    @csrf
                                    @method('POST')
                            ...
                            @else
                                <form action="{{ route('spelltype.destroy', $spellType->id) }}"
                                      method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="dropdown-item">
